...
pub fn create(device: &Device, signaled: bool) -> Result<Fence, Error> {
...
pub fn wait(device: &Device, fences: &[Fence], wait_all: bool) -> Result<(), Error> {
    unsafe { device.wait_for_fences(fences, wait_all, std::u64::MAX)? }
    Ok(())
}

pub fn reset(device: &Device, fences: &[Fence]) -> Result<(), Error> {
