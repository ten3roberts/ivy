    ...
    violet::{
        core::{
            ...
            widget::{
                ...
                IterWidgetCollection, List, LoadingSpinner, Rectangle, ScrollArea, Selectable,
        ...
        lucide::icons::{
            ...
            LUCIDE_FOLDER_OPEN, LUCIDE_LOCK, LUCIDE_PACKAGE, LUCIDE_SATELLITE, LUCIDE_TRASH_2,
        },
    },
};
...
pub fn window(
    ...
    col((window_header(icon, title, controls), card(content)))
        .with_stretch(true)
...
impl Widget for AspectInspectorPanel {
    fn mount(self, scope: &mut Scope<'_>) {
        ...
        StreamWidget::new(details.filter_map(|v| ready(v)).map(move |details| {
            ...
            window(LUCIDE_SATELLITE, "Inspector", lock_widget, panel(details))
...
impl Widget for Breadcrumbs<'_> {
    fn mount(self, scope: &mut Scope<'_>) {
        ...
        let items = self.path.components().map(|segment| {
            ...
            let widget =
                InteractiveWidget::new(pill(label(&segment_str)).with_margin(Edges::even(2.0)))
